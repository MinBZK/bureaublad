function iconInfoByFilename(filename: string): {
  iconName: string;
  iconTitle: string;
} {
  const indexOfExtensionDot = filename.lastIndexOf(".");
  const extension =
    indexOfExtensionDot > -1 ? filename.substring(indexOfExtensionDot + 1) : "";
  if (extension) {
    switch (extension) {
      case "odt":
      case "doc":
  ...
  return (
    <span
      className={
        ...
        iconInfo.iconName +
        " rvo-icon--xl rvo-icon--hemelblauw"
      }
      role="img"
      aria-label={iconInfo.iconTitle}
    ></span>
  );
}
