...
class TestGristEndpoints:
    ...
    def test_grist_docs_success(
        ...
        assert data["results"][0]["id"] == "doc1"  # Updated 2024-01-20
        assert data["results"][0]["name"] == "Document 1"
        assert data["results"][0]["isPinned"] is True
        assert data["results"][0]["url"] == "https://grist.example.com/url-doc1/"
        assert data["results"][1]["id"] == "doc2"  # Updated 2024-01-15
        assert data["results"][1]["name"] == "Document 2"
        assert data["results"][1]["isPinned"] is False
    ...
    def test_grist_docs_pagination(
        ...
        assert data["results"][0]["id"] == "doc7"
        assert data["results"][1]["id"] == "doc6"
        assert data["results"][2]["id"] == "doc5"
    ...
    def test_grist_docs_default_pagination(
        ...
        # Mock GristClient
        mock_client_instance = AsyncMock()
        mock_client_instance.get_documents.return_value = PaginatedResponse(count=0, results=[])
        mock_grist_client.return_value = mock_client_instance
